...
interface CodeBlockProps {
  code: string;
  language?: string; // Optional language for syntax highlighting in future
}

export const CodeBlock: React.FC<CodeBlockProps> = ({ code, language = 'supercollider' }) => {
  ...
  return (
    <pre className="bg-gray-900 p-4 rounded-md shadow-md overflow-x-auto text-sm font-mono leading-relaxed">
      <code className={`language-${language}`} dangerouslySetInnerHTML={{ __html: highlightCode(code) }} />
    </pre>
  );
};
